...
def build_svg(accent, halo, bg, contrast=1.0):
    svg=[]
...
''')
    ...
    for i in range(num_lines):
        ...
        svg.append(f'<path d="{d}" fill="none" stroke="{halo}" stroke-opacity="{halo_opacity:.3f}" stroke-width="{stroke_w*6:.2f}" filter="url(#softGlow)"/>')
        svg.append(f'<path d="{d}" fill="none" stroke="{accent}" stroke-opacity="{core_opacity:.3f}" stroke-width="{stroke_w:.2f}"/>')
    svg.append('</svg>')
    return "\n".join(svg)
...
if __name__ == "__main__":
    ...
    out_std.write_text(build_svg(pal["accent"], pal["halo"], pal["bg"], contrast=1.0), encoding="utf-8")
    out_subtle.write_text(build_svg(pal["accent"], pal["halo"], pal["bg"], contrast=0.8), encoding="utf-8")
    print(f"Generated: {out_std}")
    print(f"Generated: {out_subtle}")
